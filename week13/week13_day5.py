...
model = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=2) # labels fit the model

# Define optimizer and stlr scheduler
optimizer = torch.optim.Adam(model.parameters(), lr=2e-5)

# Define slanted trinagular learning rate (STLR)
num_training_steps = len(train_loader) * 3
warmup_steps = int(0.1 * num_training_steps)
scheduler = get_scheduler(
    "linear", optimizer=optimizer, num_warmup_steps=warmup_steps, num_training_steps=num_training_steps
)
# slanted triangular replaced with linear - can also do cosine
...
with torch.no_grad():
    for batch in test_loader:
        ...
        all_labels.extend(batch["labels"].cpu().numpyy())
