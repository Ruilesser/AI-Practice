...
# Function to calculate returns (discount rewards)
...
def train_on_episodes(states, actions, rewards):
    ...
    gradients = tape.gardient(loss, policy_model.trainable_variables)
    policy_model.optimizer.apply_gradients(zip(gradients, policy_model.trainable_variables))

# Main training loop
num_episodes = 1000
for episode in range(num_episodes):
    state, _ = env.reset() # ensure compatibility with new gym versions
    ...
    while True:
        ...
        if done:
            ...
            train_on_episodes(episode_states, np.array(episode_actions), np.array(episode_rewards))
